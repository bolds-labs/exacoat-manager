...
async function createPluginZip() {
  ...
  const updatedPhp = phpContent.replace(/define\(\s*['"]EXACOAT_CORE_VERSION['"],\s*['"][^'"]+['"]\s*\);/g, `define( 'EXACOAT_CORE_VERSION', '${version}' );`);
  ...
  console.log('[ZIP BUILDER] Generating UNIX-compliant ZIP with forward slashes for exacoat-core...');
  ...
  console.log('[ZIP BUILDER] Generating UNIX-compliant ZIP with forward slashes for artmatter-core...');
  const artmatterZip = new JSZip();
  const artmatterRootFolder = artmatterZip.folder('artmatter-core');
  addDirectoryToZip(pluginDir, artmatterRootFolder);
  const artmatterContent = await artmatterZip.generateAsync({
    type: 'nodebuffer',
    compression: 'DEFLATE',
    compressionOptions: { level: 9 },
    platform: 'UNIX',
  ...
  artmatterTargets.forEach(target => {
    fs.writeFileSync(target, artmatterContent);
    console.log(`[ZIP BUILDER] Wrote legacy compatibility archive to: ${path.basename(target)}`);
  });
