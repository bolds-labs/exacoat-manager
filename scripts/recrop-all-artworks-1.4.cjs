/**
 * Artmatter - Batch Artwork Recrop to 1:1.4 (32x45cm Poster Standard) Migration Runner
 * 
 * Usage:
 *   node scripts/recrop-all-artworks-1.4.cjs                # Run full migration (auto-resumes if stopped)
 *   node scripts/recrop-all-artworks-1.4.cjs --test 10106   # Test single artwork by WordPress ID
 *   node scripts/recrop-all-artworks-1.4.cjs --limit 5      # Run next 5 artworks
 *   node scripts/recrop-all-artworks-1.4.cjs --retry-failed # Retry failed artworks
 */
...
const STATE_FILE = path.join(__dirname, '.migration_ratio_1.4_state.json');
...
async function recropArtworkOnWordPress(wpId, r2Key = '') {
  ...
  try {
    let res = await fetch(WP_API_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (res.status === 404) {
      res = await fetch(WP_API_FALLBACK, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'User-Agent': 'Artmatter-Ratio-Migrator/1.0'
...
async function main() {
  ...
  console.log('\n=============================================================');
  console.log('📐 ARTMATTER RATIO MIGRATION ENGINE (1:1.4 / 32x45cm Standard)');
  console.log('=============================================================\n');

  if (testWpId) {
    console.log(`🧪 TEST MODE: Processing single artwork with WordPress ID #${testWpId}...`);
    const artworks = await fetchAllArtworks();
    const targetArt = artworks.find(a => a.wp_id === testWpId);
    const r2Key = targetArt ? getR2KeyForArtwork(targetArt) : '';
    console.log(`🔑 Resolved R2 Key: "${r2Key || 'none'}"`);
    const res = await recropArtworkOnWordPress(testWpId, r2Key);
  ...
  let queue = artworks.filter(art => {
    ...
    if (art.metadata?.aspect_ratio_standard === '1:1.4' && art.metadata?.ratio_v2_migrated) {
  ...
  if (queue.length === 0) {
    console.log('🎉 All artworks are already up to date with 1:1.4 standard ratio! No action needed.\n');
    return;
  }
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    if (result && result.success) {
      ...
      const dimChange = result.already_exact ? '(already exact 1:1.4)' : `${result.old_dimensions} ➔ ${result.new_dimensions}`;
