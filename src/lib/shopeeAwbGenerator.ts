 ...
 */
function renderShopeeAwbCard(order: ShopeeOrder): string {
  ...
  return `
    <div class="awb-wrapper">
      <div class="awb-card">
        <!-- Header: Shopee Brand, Service Type, Courier Badge -->
        <div class="header-row">
          <div class="shopee-brand">
            <svg viewBox="0 0 100 100" width="22" height="22">
              <path d="M50 10 C35 10 25 22 25 35 L75 35 C75 22 65 10 50 10 Z" fill="none" stroke="#EE4D2D" stroke-width="7"/>
              <rect x="15" y="30" width="70" height="60" rx="10" fill="#EE4D2D"/>
              <path d="M45 45 Q50 40 55 45 Q60 50 50 60 Q40 70 55 70" fill="none" stroke="#fff" stroke-width="5" stroke-linecap="round"/>
            </svg>
            <span style="font-size: 15px; font-weight: 900; color: #EE4D2D; letter-spacing: -0.5px;">Shopee</span>
          </div>
          <div class="service-type">${serviceCode}</div>
          <div>${courierBadge}</div>
        </div>

        <!-- Tracking Resi Box + Barcode -->
        <div class="resi-box">
          <div class="barcode-svg-container">${resiBarcodeSvg}</div>
          <div class="resi-number-title">No. Resi: ${trackingNumber}</div>
        </div>

        <!-- Two Columns: Recipient & Sender -->
        <div class="columns-row">
          <!-- Recipient -->
          <div class="col-left">
            <div>
              <strong>Penerima: ${recipientName}</strong>
              <span class="badge-pill">RUMAH</span>
            </div>
            <div style="font-size: 9px; margin-top: 1px;">${recipientPhone}</div>
          ...
          <!-- Sender -->
          <div class="col-right">
            <div><strong>Pengirim: Exacoat Indonesia</strong></div>
            <div style="font-size: 9px; margin-top: 1px;">6281210008800</div>
            <div class="address-text">
              Exacoat HQ Fulfillment, Ruko Ruby Commercial, Summarecon Bekasi, KOTA BEKASI - JAWA BARAT
            </div>
          </div>
        </div>
...
<head>
  ...
  <style>${SHOPEE_COMMON_CSS}</style>
</head>
<body>
  ${renderShopeeAwbCard(order)}
  <script>
...
export function generateShopeeBatchAwbHtml(orders: ShopeeOrder[]): string {
  const cards = orders.map((order) => renderShopeeAwbCard(order)).join('');
  return `<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <title>Shopee Batch AWBs (${orders.length} orders)</title>
  <style>${SHOPEE_COMMON_CSS}</style>
</head>
<body>
  ${cards}
